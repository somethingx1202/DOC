...
def load_and_prepare_train(root=""):
    df = pd.read_csv(root + "CMF_train_r_pairwise.csv")
...
def load_and_prepare_test(root=""):
    df = pd.read_csv(root + 'CMF_test_r_pairwise.csv')
...
def token_level_to_char_level(text, offsets, preds):
    # The text is simply a string
    probas_char = np.zeros(len(text))
    # assert max(offsets) == len(probas_char)
    # print(np.max(offsets))
    # assert np.max(offsets) <= 120
    # Problem happens here: the text has been trimmed, it's no longer real
    for i, offset in enumerate(offsets):
        # if offset[0] != 0 or offset[1] != 0, which means that they are not special tokens
        # So the first 3 are omitted and the last is also omitted.
        # The probabilities are the starting probabilities
        if i == 25:
            print('yiiiii?', preds[i], offset, probas_char[offset[0]:offset[1]])
        if offset[0] or offset[1]:
            # remove padding and sentiment
            # print(preds.shape)
            probas_char[offset[0]:offset[1]] = preds[i]
        if offset[0] <= 81 and offset[1] >= 82:
            print('happened at %d' % (i))
        if i == 26:
            print('yiiiii? %d?' % (i), preds[i], offset, probas_char[offset[0]:offset[1]])
            print(text)
        if i == 25:
            print('yiiiii?', preds[i], offset, probas_char[offset[0]:offset[1]])
    pred = np.argmax(preds)
    print('whatwhatwhat??', probas_char[81:82])
    pred_char = np.argmax(probas_char)
    print(offsets)
    print(preds)
    print(probas_char)
    print(pred)
    print(offsets[pred][0])
    print(pred_char)
    print(preds[pred])
    print(probas_char[pred_char])
    assert pred_char == offsets[pred][0]
    return probas_char


def get_char_preds_eval(pred_tests, train_dataset, fpathout, fpathout_semb):
    mf1 = MicroF1Meter()
    mf1_se = MicroF1Meter()
    mf1_st = MicroF1Meter()
    pred_test_clss = []
    ...
    char_pred_test_end = []
    pred_test_sEmbeds = []
    # len(test_dataset) is the nb_of_instances
    # fp = open('preds.txt', 'wt', encoding='utf8')
    lst_for_csv_output = []
    # lst_for_csv_semb_output = []
    lst_for_dat_semb_output = []
    tps = []
    fns = []
    fps = []
    tps_se = 0
    ...
    for idx in range(len(train_dataset)):
        ...
        # i is the k_fold, so needs to do a mean, then no longer the softmax, since it has been averaged
        # currently it's pos_logits_sgmd averaged
        # due to [idx] it further becomes position_pred_test_a
        ...
        # 2022/08/08
        sEmbed_preds = np.mean([pred_tests[i][3][idx] for i in range(len(pred_tests))], 0)
        # print('type of sEmbed_preds is', type(sEmbed_preds))
        # eval_fn uses sort, here we use argmax
        start_pred = np.argmax(start_preds)
        end_pred = np.argmax(end_preds)

        # single_char_pred_test_start_lst = token_level_to_char_level(text, offsets, start_preds)
        # char_pred_test_start.append(single_char_pred_test_start_lst)
        # single_char_pred_test_end_lst = token_level_to_char_level(text, offsets, end_preds)
        # char_pred_test_end.append(single_char_pred_test_end_lst)
        pred_test_clss.append(cls_preds)
        # print(sEmbed_preds.shape) # 768
        pred_test_sEmbeds.append(sEmbed_preds)

        # post process would require a strip for the possible precursory spaces before the first token.
        # That is, post_process_spaces
        
        # values = single_char_pred_test_start_lst
        # max_index_pred_test_start = max(range(len(values)), key=values.__getitem__)
        # assert max_index_pred_test_start == offsets[start_pred][0]

        # values = single_char_pred_test_end_lst
        # max_index_pred_test_end = np.argmax(values[::-1])
        # max_index_pred_test_end = len(values) - max_index_pred_test_end

        # Actually, there's no restriction on the length of the text, there is only check on the ids length, so althought prediction will always fall into the [0, 120] interval (due to the "minus maxarg setup the prediction will fall outside of the text and will no longer be accurate since len(text) is no longer 120")
        #     But this also shows that prediction may be stretch to > 120, during the code scrutinization I believe it is due to the 'all-zero' prediction. 
        # if max_index_pred_test_end > 120:
        #     print('what? > 120?!', external_id)
        #     print(len(values))
        #     print(max_index_pred_test_end)
        #     print(values[-1])
        # char_pred_test_a = char_pred_test_a[:len(text)]
        # char_predsTorF_a = char_pred_test_a > 0.5
        # char_preds_spcdeld_a = post_process_spaces(char_predsTorF_a, text)
        # spans_pred_a = char_target_to_span(char_preds_spcdeld_a)

        st_pred = np.argmax(cls_preds)

        # # Just for test round 3, 1
        # print(text[max_index_pred_test_start: max_index_pred_test_end])
        # print(text[offsets[start_pred][0]:offsets[end_pred][1]])
        # assert text[max_index_pred_test_start: max_index_pred_test_end] == text[offsets[start_pred][0]:offsets[end_pred][1]]
        lst_for_csv_output.append(
            [external_id, text, st_pred, text[offsets[start_pred][0]:offsets[end_pred][1]]])
        # lst_for_csv_semb_output.append(
        #     [external_id, sEmbed_preds])
        lst_for_dat_semb_output.append(
            sEmbed_preds
        )
        # I know what the error is. The spans_pred wants the token index instead of char index, caussing the offsets out-of-index
        ...
        if st_pred == stance_cate:
            ...
            fns_st += 1
            fps_st += 1
        # fp.write(text[max_index_pred_test_start: max_index_pred_test_end] + '\n')
    # Of course you can use pandas, just remember to set the right path
    ...
    fp.close()
    # 2022/08/09
    # fp_semb = open(fpathout_semb, 'wt', encoding='utf8')
    # # from the bookmarks we can see that this doesn't allow \'
    # fp_semb.write(','.join(["ID", "sembed"]) + '\n')
    # # If we use savetxt then we would encounter nested array, which is unfavorable to handle
    # np.savetxt(fp_semb, np.asarray(lst_for_csv_semb_output), '%s', ',')
    # fp_semb.close()
    arr_semb = np.asarray(lst_for_dat_semb_output, dtype=np.float32)
    # print(arr_semb.shape) # (800, 768)
    arr_semb.tofile(fpathout_semb)

    # reshape can take in tuple or several arguments
    arr_semb_rec = np.fromfile(fpathout_semb, dtype=np.float32).reshape(arr_semb.shape)
    print(np.array_equal(arr_semb, arr_semb_rec))
...
class ConfigAlbert:
    ...
    out_path = f"../input/doccheckpoints/5-fold-220727/{selected_model}"
    # weights_path = f'../input/doccheckpoints/5-fold-220804/{selected_model}'
    weights_path = f'../input/doccheckpoints/5-fold-220727/{selected_model}'
...
def clustering():
    ...
    filename = '../input/datasets/CMF_test_r.csv'
